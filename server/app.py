...
One process that serves the map and answers chat.
  ...
  /api/ward-report?ward=3:43 -> a PDF report for one ward
...
from .report_pdf import ward_report_pdf
...
def ward_report(ward: str):
    ...
    web_results = None  # None: search not configured; []: no results
    ...
    pdf = ward_report_pdf(feature, points_payload, web_results)

    filename = f"ward-report-{ward.replace(':', '-')}.pdf"
    return Response(
        content=pdf,
